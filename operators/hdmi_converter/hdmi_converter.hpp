...
class HDMIConverterOp : public holoscan::Operator {
public:
    ...
    uint32_t receiver_start_byte();
    uint32_t received_line_bytes(uint32_t line_bytes);
    uint32_t transmitted_line_bytes(PixelFormat pixel_format, uint32_t pixel_width);
    void configure(uint32_t start_byte, uint32_t bytes_per_line, uint32_t pixel_width, uint32_t pixel_height, PixelFormat pixel_format, uint32_t trailing_bytes);
...
private:
    ...
    std::shared_ptr<holoscan::common::CudaFunctionLauncher> cuda_function_launcher_;
    ...
    std::unique_ptr<nvidia::gxf::MemoryBuffer> left_eye_device_scratch_buffer_;
    std::unique_ptr<nvidia::gxf::MemoryBuffer> right_eye_device_scratch_buffer_;
    std::unique_ptr<nvidia::gxf::MemoryBuffer> rgb_resize_buffer_;

    std::chrono::time_point<std::chrono::high_resolution_clock> last_time_;
    uint32_t pixel_width_ = 0;
    uint32_t pixel_height_ = 0;
    PixelFormat pixel_format_ = PixelFormat::RAW_8;
