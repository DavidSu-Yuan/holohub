...
#ifdef QCAP_HSB
      auto hsb = make_operator<ops::QcapHSBOp>("qcap_hsb", from_config("qcap_hsb"));
...
#else
      throw std::runtime_error(
          "qcap_hsb is requested but not available.");
#endif
...
bool parse_arguments(int argc, char** argv, std::string& config_name, std::string& data_path, bool& fullscreen, bool& vsync) {
  ...
  while ((c = getopt_long(argc, argv, "d:fv", long_options, &option_index)) != -1) {
      switch (c) {
          case 'd':
              data_path = optarg;
              break;
          ...
          case 0: {
                      // for options that set flag (not used here)
                      const struct option* cur_opt = &long_options[option_index];
                      if (strcmp(cur_opt->name, "fullscreen") == 0) {
                          fullscreen = true;
                      }
                      break;
                  }
...
int main(int argc, char** argv) {
  ...
  bool vsync = false;
  if (!parse_arguments(argc, argv, config_name, data_path, fullscreen, vsync)) { return 1; }
