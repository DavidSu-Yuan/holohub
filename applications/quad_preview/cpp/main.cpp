...
class App : public holoscan::Application {
 public:
  ...
  void compose() override {
    ...
    std::shared_ptr<Operator> left_top_source;
    std::shared_ptr<Operator> right_top_source;
    std::shared_ptr<Operator> left_bottom_source;
    std::shared_ptr<Operator> right_bottom_source;
    ...
    if (source_ == "yuan") {
      ...
      height = 2160;
#ifdef YUAN_QCAP
      left_top_source = make_operator<ops::QCAPSourceOp>("yuan_left_top", from_config("yuan_left_top"), Arg("image_directory", imagepath));
      right_top_source = make_operator<ops::QCAPSourceOp>("yuan_right_top", from_config("yuan_right_top"), Arg("image_directory", imagepath));
      left_bottom_source = make_operator<ops::QCAPSourceOp>("yuan_left_bottom", from_config("yuan_left_bottom"), Arg("image_directory", imagepath));
      right_bottom_source = make_operator<ops::QCAPSourceOp>("yuan_right_bottom", from_config("yuan_right_bottom"), Arg("image_directory", imagepath));
#endif
	...
	ops::HolovizOp::InputSpec::View left_top_view;
	left_top_view.offset_x_ = 0;
	left_top_view.offset_y_ = 0;
	left_top_view.width_ = 0.5;
	left_top_view.height_ = 0.5;

	ops::HolovizOp::InputSpec left_top_spec("Left_Top", ops::HolovizOp::InputType::COLOR);
	left_top_spec.views_.push_back(left_top_view);
	specs.push_back(left_top_spec);

	ops::HolovizOp::InputSpec::View right_top_view;
	right_top_view.offset_x_ = 0.5;
	right_top_view.offset_y_ = 0;
	right_top_view.width_ = 0.5;
	right_top_view.height_ = 0.5;

	ops::HolovizOp::InputSpec right_top_spec("Right_Top", ops::HolovizOp::InputType::COLOR);
	right_top_spec.views_.push_back(right_top_view);
	specs.push_back(right_top_spec);

	ops::HolovizOp::InputSpec::View left_bottom_view;
	left_bottom_view.offset_x_ = 0;
	left_bottom_view.offset_y_ = 0.5;
	left_bottom_view.width_ = 0.5;
	left_bottom_view.height_ = 0.5;

	ops::HolovizOp::InputSpec left_bottom_spec("Left_Bottom", ops::HolovizOp::InputType::COLOR);
	left_bottom_spec.views_.push_back(left_bottom_view);
	specs.push_back(left_bottom_spec);

	ops::HolovizOp::InputSpec::View right_bottom_view;
	right_bottom_view.offset_x_ = 0.5;
	right_bottom_view.offset_y_ = 0.5;
	right_bottom_view.width_ = 0.5;
	right_bottom_view.height_ = 0.5;

	ops::HolovizOp::InputSpec right_bottom_spec("Right_Bottom", ops::HolovizOp::InputType::COLOR);
	right_bottom_spec.views_.push_back(right_bottom_view);
	specs.push_back(right_bottom_spec);
	...
	add_flow(left_top_source, visualizer_operator, {{output_signal, input_video_signal}});
	add_flow(right_top_source, visualizer_operator, {{output_signal, input_video_signal}});
	add_flow(left_bottom_source, visualizer_operator, {{output_signal, input_video_signal}});
	add_flow(right_bottom_source, visualizer_operator, {{output_signal, input_video_signal}});
    } else {
	visualizer_operator = make_operator<ops::HolovizOp>(
			"holoviz",
			from_config(overlay_enabled ? "holoviz_overlay" : "holoviz"),
